...
const MembersCategory: React.FC<Props> = ({ category, members }) => {
    return (
        <div className="mb-4">
            <h5 className="font-semibold uppercase text-xs mb-2">
                {category} - {members.length}
            </h5>
            {members.map(member => (
                <div
                    className="flex items-center cursor-pointer hover:bg-gray-700 py-2 rounded-md"
                    key={member.name}
                >
